...
return {
	name = "dinit start",
	description = "Start a dinit service when it is not already started.",
	...
	apply = function(ctx, args)
		if exec(ctx, "dinitctl", { "--quiet", "is-started", args.service }, args.timeout).ok then
			return command_result(
				"unchanged",
				"dinitctl start " .. args.service,
				"service is already active",
				{ service = args.service, action = "start" }
			)
		end
		local argv = { "start", args.service }
