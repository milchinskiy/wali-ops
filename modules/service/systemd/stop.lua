...
return {
	name = "systemd stop",
	description = "Stop a systemd unit when it is active.",
	...
	apply = function(ctx, args)
		if not exec(ctx, "systemctl", { "is-active", "--quiet", args.unit }, args.timeout).ok then
			return command_result(
				"unchanged",
				"systemctl stop " .. args.unit,
				"unit is already inactive",
				{ unit = args.unit, action = "stop" }
			)
		end
		local argv = { "stop", args.unit }
