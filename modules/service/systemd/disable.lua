...
return {
	...
	description = "Disable a systemd unit when it is enabled.",
	...
	apply = function(ctx, args)
		if not exec(ctx, "systemctl", { "is-enabled", "--quiet", args.unit }, args.timeout).ok then
			return command_result(
				"unchanged",
				"systemctl disable " .. args.unit,
				"unit is already disabled",
				{ unit = args.unit, action = "disable" }
			)
		end
		local argv = { "disable", args.unit }
