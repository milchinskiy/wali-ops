...
return {
	name = "systemd enable",
	description = "Enable a systemd unit when it is not already enabled.",
	...
	apply = function(ctx, args)
		if exec(ctx, "systemctl", { "is-enabled", "--quiet", args.unit }, args.timeout).ok then
			return command_result(
				"unchanged",
				"systemctl enable " .. args.unit,
				"unit is already enabled",
				{ unit = args.unit, action = "enable" }
			)
		end
		local argv = { "enable", args.unit }
