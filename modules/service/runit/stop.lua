...
return {
	name = "runit stop",
	description = "Stop a runit service with sv down when it is running.",
	requires = { command = "sv" },
	...
	apply = function(ctx, args)
		...
		local status = exec(ctx, "sv", { "status", path }, args.timeout)
		local text = status.stdout or status.output or ""
		if not (status.ok and text:sub(1, 4) == "run:") then
			return command_result(
				"unchanged",
				"sv down " .. path,
				"service is already inactive",
				{ service = args.service, path = path, action = "stop" }
			)
		end
		local argv = { "down", path }
