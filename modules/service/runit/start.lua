...
return {
	name = "runit start",
	description = "Start a runit service with sv up when it is not already running.",
	requires = { command = "sv" },
	...
	apply = function(ctx, args)
		...
		if status.ok and text:sub(1, 4) == "run:" then
			return command_result(
				"unchanged",
				"sv up " .. path,
				"service is already active",
				{ service = args.service, path = path, action = "start" }
			)
		end
		local argv = { "up", path }
