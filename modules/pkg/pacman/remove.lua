...
return {
	name = "pacman remove",
	description = "Remove one or more Arch packages with pacman when installed.",
	...
	apply = function(ctx, args)
		...
		if #installed == 0 then
			return command_result("unchanged", "pacman -R", "all packages are already absent", args.packages, "remove")
		end
		local argv = { args.recursive and "-Rs" or "-R", "--noconfirm" }
		if args.nosave then
			table.insert(argv, "--nosave")
		end
		for _, name in ipairs(installed) do
			table.insert(argv, name)
		end
		checked(ctx, "pacman", argv, args.timeout)
		return command_result("updated", command_detail("pacman", argv), "removed packages", installed, "remove")
	end,
}
