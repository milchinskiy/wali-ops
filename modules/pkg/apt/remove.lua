...
return {
	name = "apt remove",
	description = "Remove one or more Debian packages with apt-get when installed.",
	...
	apply = function(ctx, args)
		...
		local argv = { args.purge and "purge" or "remove", "-y", "--" }
		for _, name in ipairs(installed) do
			table.insert(argv, name)
		end
		checked(ctx, "apt-get", argv, args.timeout, env)
		if args.autoremove then
			checked(ctx, "apt-get", { "autoremove", "-y" }, args.timeout, env)
		end
		return command_result("updated", command_detail("apt-get", argv), "removed packages", installed, "remove")
	end,
}
