...
local function is_installed(ctx, name, timeout)
	return exec(ctx, "xbps-query", { name }, timeout).ok
end

return {
	name = "xbps remove",
	description = "Remove one or more Void packages with xbps-remove when installed.",
	requires = { all = { { command = "xbps-remove" }, { command = "xbps-query" } } },
	...
	apply = function(ctx, args)
		...
		if #installed == 0 then
			return command_result(
				"unchanged",
				"xbps-remove",
				"all packages are already absent",
				args.packages,
				"remove"
			)
		end
		...
		checked(ctx, "xbps-remove", argv, args.timeout)
		return command_result("updated", command_detail("xbps-remove", argv), "removed packages", installed, "remove")
	end,
}
